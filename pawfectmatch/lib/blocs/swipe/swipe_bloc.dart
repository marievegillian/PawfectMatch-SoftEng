...
import 'package:pawfectmatch/widgets/matched_popup.dart';
import 'package:geolocator/geolocator.dart';
import 'package:pawfectmatch/utils/dog_ranking.dart'; // Ensure correct import
part 'swipe_event.dart';
part 'swipe_state.dart';
  ...
  try {
    ...
    print('Filtered Dogs:');
      for (var dog in filteredDogs) {
        print('Name: ${dog.name}, Breed: ${dog.breed}, Age: ${dog.calculateAge()}, Gender: ${dog.fetchGender()}');
      }
...
void _onSwipeRight(
  ...
  if (state is SwipeLoaded) {
    ...
    if (dogs.isNotEmpty) {
      ...
      // Prepare user preferences based on swiped dog's data
      Dog swipedDog = event.dogs;
      Map<String, dynamic> userPreferences = {
        "activities": swipedDog.activities,
        "breed": swipedDog.breed,
        "size": swipedDog.size
      };

      // rankDogs function
      print('YOUVE SWIPED RIGHT TO SUMMON DOG RANKING');
      List<Dog> rankedDogs = rankDogs(userPreferences, dogs);

      // Update state with ranked dogs
      emit(SwipeLoaded(dogs: rankedDogs));

      // Firestore updates
      String likedDogId = swipedDog.dogId;
      await _databaseRepository.updateLikedDogsInFirestore(likedDogId); 

      bool isMatch = await _databaseRepository.checkMatch(likedDogId);
      if (isMatch) {
        await _databaseRepository.updateMatched(likedDogId);
        _showMatchedPopup(event.context, swipedDog.profilePicture);

        // Create a conversation
        String loggedInDog = _databaseRepository.loggedInDogUid;
        await _databaseRepository.createConversation(loggedInDog, likedDogId);
      }
    } else {
      emit(SwipeError());
    }
