...
def calculate_bio_similarity(bio, traits):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.to(device)

    tokens = tokenizer(bio, return_tensors='pt', truncation=True, padding=True)
    tokens = {key: value.to(device) for key, value in tokens.items()}

    with torch.no_grad():
        bio_embedding = model(**tokens).last_hidden_state.mean(dim=1).cpu().numpy()

    # Convert traits to a single string and get its embedding
    traits_text = ' '.join(traits)
    traits_tokens = tokenizer(traits_text, return_tensors='pt', truncation=True, padding=True)
    traits_tokens = {key: value.to(device) for key, value in traits_tokens.items()}

    with torch.no_grad():
        traits_embedding = model(**traits_tokens).last_hidden_state.mean(dim=1).cpu().numpy()

    # Compute cosine similarity
    similarity = cosine_similarity(bio_embedding, traits_embedding)
...
def filter_and_rank_dogs(user_input, df):
    ...
    df_filtered['bio_similarity'] = df_filtered['bio'].apply(lambda bio: calculate_bio_similarity(bio, traits))

    # Sort by similarity score
    df_filtered = df_filtered.sort_values(by='bio_similarity', ascending=False)
    return df_filtered

@app.route('/filter_dogs', methods=['POST'])
def filter_dogs():
